...
async function getFeaturedProducts() {
  try {
    ...
    return products.filter((p: { featured: boolean }) => p.featured).slice(0, 3);
  } catch {
    return [];
  }
}
        ...
        <section className="py-16 border-t border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
            {[
              ...
              <div key={label} className="border border-border rounded-md text-center py-6 px-4">
        ...
        <section className="py-16 border-t border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {featuredProducts.length > 0 ? (
              featuredProducts.map((p: { slug: string; name: string; price: number }) => (
                <ProductCard
                  key={p.slug}
                  slug={p.slug}
                  name={p.name}
              ...
              <p className="text-foreground-faint text-sm">No featured products yet.</p>
            )}
          </div>
        </section>
